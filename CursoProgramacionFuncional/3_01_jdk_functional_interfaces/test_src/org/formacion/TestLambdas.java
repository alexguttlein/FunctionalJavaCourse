...
public class TestLambdas {
	...
	public void test_extras() {
		...
		Predicate<Persona> sinSegundoApellido = p -> p.getApellido2() == null;

		assertTrue(sinSegundoApellido.test(personaSinSegundoApellido));
		assertFalse(sinSegundoApellido.test(personaConSegundoApellido));


		// Una funcion que nos diga si dos personas son parientes: para nosotros parientes
		// son personas con el mismo primer apellido
		BiPredicate<Persona, Persona> sonParientes = (a,b) -> a.getApellido1().equals(b.getApellido1());
		...
		// y nombre
		UnaryOperator<Persona> enmascarar = pers -> new Persona (
				pers.getApellido2(),
				pers.getNombre(),
				pers.getApellido1()
		);

		Persona enmascarada = enmascarar.apply(personaConSegundoApellido);
		assertEquals("apellido2", enmascarada.getNombre());
		assertEquals("nombre", enmascarada.getApellido1());
		assertEquals("apellido1", enmascarada.getApellido2());
	}
	...
	public void test_validadores() {
		...
		validador.add(p -> p.getNombre() != null);
		...
		assertFalse(validador.valida(new Persona(null,"ape1","ape2")));
